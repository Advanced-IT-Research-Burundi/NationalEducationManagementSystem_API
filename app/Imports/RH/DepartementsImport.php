...
class DepartementsImport implements ToCollection, WithHeadingRow, SkipsEmptyRows
{
    ...
    public function collection(Collection $rows): void
    {
        foreach ($rows as $index => $row) {
            try {
                ...
                $payload = [
                    'code' => $code ?: 'DEP-' . str_pad((string) ($index + 1), 4, '0', STR_PAD_LEFT),
                    'nom' => $nom,
                ...
                if (! empty($payload['code'])) {
                    $existing = Departement::query()->where('code', $payload['code'])->first();
                }
                if (! $existing) {
                    $existing = Departement::query()->whereRaw('LOWER(TRIM(nom)) = ?', [mb_strtolower($payload['nom'])])->first();
                }
